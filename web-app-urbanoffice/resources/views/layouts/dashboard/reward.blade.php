            ...
            <div class="text-white">
                <h3 class="font-semibold text-base sm:text-lg">
                    ...
                    @if($transaction->room_type === 'Virtual Office')
                        {{ $transaction->bulan }}/{{ $transaction->tahun }}
                    @endif
                </h3>
                <p class="text-xs sm:text-sm">{{ $transaction->order_id }} | {{ optional($transaction->booking_date)->format('d-m-Y') }}</p>
            </div>
            <div class="@if($transaction->status === 'settlement') bg-green-500
                        @elseif($transaction->status === 'pending') bg-yellow-400
                        @else bg-red-500 @endif text-white px-3 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm font-medium animate-pulse">
        ...
        <div class="p-4 sm:p-6">
            <div class="flex flex-col lg:flex-row lg:items-start space-y-4 lg:space-y-0 lg:space-x-4">
                ...
                <div class="flex-1 space-y-4">
                    <h4 class="text-lg sm:text-xl font-bold text-gray-800">{{ $transaction->city }}</h4>
                    ...
                    <div class="space-y-4 sm:space-y-0 sm:grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 sm:gap-4 lg:gap-6">
                        <div class="bg-gray-50 p-3 rounded-lg">
                            ...
                            <p class="text-gray-800 font-medium">{{ $transaction->room_type }}</p>
                        </div>

                        <div class="bg-gray-50 p-3 rounded-lg">
                            <h5 class="text-sm font-semibold text-gray-600 mb-2">Pax</h5>
                            <p class="text-gray-800 font-medium">{{ $transaction->jumlah_orang ?? '-' }} pax</p>
                        </div>
                        ...
                        <div class="bg-gray-50 p-3 rounded-lg md:col-span-2 lg:col-span-1">
                            ...
                            <p class="text-gray-800 font-medium text-sm">
                                @if($transaction->room_type === 'Virtual Office')
                                    {{ $transaction->bulan ?? '-' }} bulan / {{ $transaction->tahun ?? '-' }} tahun
                                @else
                                    {{ $transaction->jam ?? '-' }} jam
                                @endif
                            </p>
                    ...
                    <div class="bg-orange-50 p-3 rounded-lg">
                        <p class="text-gray-700 text-sm sm:text-base">
                            {{ $transaction->room_type }} room {{ $transaction->jumlah_orang ?? '-' }} pax include CB
                        </p>
                    </div>
                </div>
