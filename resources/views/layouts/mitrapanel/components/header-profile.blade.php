...
@php
    $user = auth()->user();
    $initial = strtoupper(substr($user->name ?? 'M', 0, 1));
@endphp
        ...
        <div class="relative">
            <button 
                ...
                <div class="relative">
                    ...
                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-medium border border-gray-200 text-sm {{ ($user->profile_photo ?? false) ? 'hidden' : '' }}">
                        {{ $initial }}
                    </div>
            ...
            <div x-show="openProfileDropdown"
                ...
                {{-- User Info --}}
                <div class="px-3 py-2 bg-gradient-to-r from-orange-500 to-red-600 text-white">
                    <p class="text-sm font-semibold leading-tight">{{ $user->name ?? 'Nama Mitra' }}</p>
                    <p class="text-xs opacity-90 leading-tight mt-0.5">{{ $user->email ?? 'email@mitra.com' }}</p>
                </div>
