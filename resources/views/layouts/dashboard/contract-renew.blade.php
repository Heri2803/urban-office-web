...
<div class="max-w-3xl mx-auto py-8 px-4" x-data="renewForm()" x-init="init()">
    ...
    <nav class="flex mb-6 text-sm text-gray-500">
        ...
        <span class="text-gray-700">Perpanjang</span>
    </nav>

    <h1 class="text-2xl font-bold text-gray-800 mb-6">Perpanjang Kontrak</h1>

    {{-- Detail Kontrak Saat Ini --}}
    <div class="bg-white rounded-xl border border-gray-200 p-6 mb-6 shadow-sm">
        ...
        <dl class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
                <dt class="text-gray-500 text-xs uppercase tracking-wider">No. Kontrak</dt>
                <dd class="font-medium text-gray-800">{{ $contract->contract_number ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-gray-500 text-xs uppercase tracking-wider">Tipe</dt>
                <dd class="font-medium text-gray-800">{{ $contract->type }}</dd>
            </div>
            <div>
                <dt class="text-gray-500 text-xs uppercase tracking-wider">Lokasi</dt>
                <dd class="font-medium text-gray-800">{{ $contract->transaction->location->name ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-gray-500 text-xs uppercase tracking-wider">Status</dt>
                <dd>{!! $contract->status_badge !!}</dd>
            </div>
            <div>
                <dt class="text-gray-500 text-xs uppercase tracking-wider">Tanggal Mulai</dt>
                <dd class="font-medium text-gray-800">{{ $contract->formatted_start_date ?? '-' }}</dd>
            </div>
            <div>
                <dt class="text-gray-500 text-xs uppercase tracking-wider">Tanggal Berakhir</dt>
                <dd class="font-medium {{ $contract->status === 'expired' ? 'text-red-600' : 'text-gray-800' }}">
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('renewForm', () => ({
            ...
            async submitForm() {
                ...
                try {
                    const response = await fetch('{{ route("customer.contracts.renew.process", $contract) }}', {
                        method: 'POST',
                        headers: {
